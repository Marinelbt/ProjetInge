...
data_point <- data %>% 
  ...
  dplyr::select(-c(match_joue, total_point, points))
...
data_diff_norme <- data_point %>%
  dplyr::select(saison, division, HF, journee, code_rencontre, diff_point, diff_point_norm) %>% 
  na.omit()
```
...
```{r}
write.csv(data_diff_norme, "../data_final/data_diff_norme_final.csv")
```
