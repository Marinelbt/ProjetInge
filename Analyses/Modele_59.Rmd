---
title: "Modèles à 59 minutes v2"
author: "Amélie Brejot"
date: "2024-12-24"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


# Import

```{r}
library(readr)
library(dplyr)
library(tidyverse)
library(ggplot2)
library(RcmdrMisc)
library(GGally)
```

# Import données

```{r}
data <- read.csv("../data_final/df_pdfs_final.csv", header = TRUE, sep = ",")
```

# Préparation données

```{r}
data <- data %>%
  separate(score_final, into = c("score_final_r", "score_final_v"), sep = "-") %>%
  mutate(
    score_final_r = as.integer(score_final_r),
    score_final_v = as.integer(score_final_v)
  )

data_mod1 <- data %>%
  group_by(code_rencontre, club_recevant, coach_recevant, club_visiteur, coach_visiteur, journee, score_final_r, score_final_v, fichier) %>%
  summarise(temps_proche_59_min = max(Temps_Sec[Temps_Sec < 59 * 60], na.rm = TRUE),
            statut_59_min_r = first(statut_recevant[Temps_Sec == temps_proche_59_min]),
            statut_59_min_v = -statut_59_min_r,
            score_recevant_59_min = first(score_recevant[Temps_Sec == temps_proche_59_min]),
            score_visiteur_59_min = first(score_visiteur[Temps_Sec == temps_proche_59_min]),
            TM_derniere_min_r = if_else(
              any(Temps_Sec >= 59 * 60 & grepl("tem", action, ignore.case = TRUE) & action_equipe == "r"),
              1, 
              0
              ),
            TM_derniere_min_v = if_else(
              any(Temps_Sec >= 59 * 60 & grepl("tem", action, ignore.case = TRUE) & action_equipe == "v"),
              1, 
              0
              ),
            DM_r = if_else(
              any(Temps_Sec >= 57 * 60 & (grepl("2MN", action, ignore.case = TRUE) | grepl("Dis", action, ignore.case = TRUE)) & action_equipe == "r"),
              1, 
              0
              ),
            DM_v = if_else(
              any(Temps_Sec >= 57 * 60 & (grepl("2MN", action, ignore.case = TRUE) | grepl("Dis", action, ignore.case = TRUE)) & action_equipe == "v"),
              1, 
              0
              ),
            .groups = "drop"
            ) %>%
  filter(abs(score_recevant_59_min - score_visiteur_59_min) < 3) %>% 
  pivot_longer(
    cols = c(statut_59_min_r, statut_59_min_v), 
    names_to = "equipe", 
    values_to = "statut_59_min"
  ) %>%
  mutate(
    equipe = case_when(
      equipe == "statut_59_min_r" ~ "R",
      equipe == "statut_59_min_v" ~ "V"
    ),
    statut_final = case_when(
      equipe == "R" & score_final_r > score_final_v ~ 1,
      equipe == "V" & score_final_v > score_final_r ~ 1,
      score_final_v == score_final_r ~ 0,
      equipe == "R" & score_final_r < score_final_v ~ -1,
      equipe == "V" & score_final_v < score_final_r ~ -1,
    ),
    effet_positif = case_when(
      statut_59_min == -1 & (statut_final == 0 | statut_final == 1) ~ 1,
      statut_59_min == 1 & statut_final == 1 ~ 1,
      statut_59_min == 0 & statut_final == 1 ~ 1,
      TRUE ~ 0
    ),
    TM_derniere_min = case_when(
      equipe == "R" & TM_derniere_min_r == 1 ~ 1,
      equipe == "V" & TM_derniere_min_v == 1 ~ 1,
      TRUE ~ 0
    ),
    TM_derniere_min_adverse = case_when(
      equipe == "V" & TM_derniere_min_r == 1 ~ 1,
      equipe == "R" & TM_derniere_min_v == 1 ~ 1,
      TRUE ~ 0
    ),
    HF = case_when(
      grepl("F", fichier) ~ "F",
      TRUE ~ "H"
    ),
     DM = case_when(
      equipe == "R" & DM_r == 1 ~ 1,
      equipe == "V" & DM_v == 1 ~ 1,
      TRUE ~ 0
    ),
    DM_adverse = case_when(
      equipe == "V" & DM_r == 1 ~ 1,
      equipe == "R" & DM_v == 1 ~ 1,
      TRUE ~ 0
    ),
    division = case_when(
      grepl("D1", fichier, ignore.case = TRUE) ~ "D1",
      grepl("D2", fichier, ignore.case = TRUE) ~ "D2",
      grepl("N1", fichier, ignore.case = TRUE) ~ "N1"
    ),
    saison = case_when(
      grepl("2021", fichier, ignore.case = TRUE) ~ "2021",
      grepl("2122", fichier, ignore.case = TRUE) ~ "2122",
      grepl("2223", fichier, ignore.case = TRUE) ~ "2223",
      grepl("2324", fichier, ignore.case = TRUE) ~ "2324",
      grepl("2425", fichier, ignore.case = TRUE) ~ "2425"
    )
  ) %>% 
  dplyr::select(-c(TM_derniere_min_r, TM_derniere_min_v, DM_r, DM_v, temps_proche_59_min)) %>% 
  ungroup() %>% 
  filter (saison != 2425)

# On ne garde que l'équipe recevante 
data_mod1 <- data_mod1 %>%
  filter(equipe == "R") %>% 
  filter(statut_59_min != 0 | statut_final != 0)

```

# Mise en facteur

```{r}
data_mod1$statut_59_min <- factor(data_mod1$statut_59_min)
data_mod1$statut_final <- factor(data_mod1$statut_final)
data_mod1$effet_positif <- factor(data_mod1$effet_positif)
data_mod1$TM_derniere_min <- factor(data_mod1$TM_derniere_min)
data_mod1$TM_derniere_min_adverse <- factor(data_mod1$TM_derniere_min_adverse)
data_mod1$HF <- factor(data_mod1$HF)
data_mod1$DM <- factor(data_mod1$DM)
data_mod1$DM_adverse <- factor(data_mod1$DM_adverse)
data_mod1$club_recevant <- factor(data_mod1$club_recevant)
data_mod1$saison <- factor(data_mod1$saison)
data_mod1$division <- factor(data_mod1$division)
```


# Ajout de la variable différence de point

```{r}
diff_norme <- read.csv("../data_final/data_diff_norme_final.csv", row.names = 1)
diff_norme$saison <- factor(diff_norme$saison)

data_mod1 <- data_mod1 %>% 
  dplyr::select(-c("journee","division","HF", "saison")) %>% 
  left_join(diff_norme, by = join_by(code_rencontre)) %>% 
  drop_na() # NA dû à l'absence de certaines fdm
```

```{r}
data_mod1 %>% 
  ggplot(aes(x = diff_point_norm)) +
  geom_density() +
  theme_minimal()

quantiles <- quantile(data_mod1$diff_point_norm, probs = c(0, 1/3, 2/3, 1))

print(quantiles)

data_mod1$group_diff_point <- cut(data_mod1$diff_point_norm,
                                  breaks = quantiles,
                                  labels = c("moins fort","meme niveau","plus fort"),
                                  include.lowest = T)
table(data_mod1$group_diff_point)


ggplot(data_mod1, aes(x = group_diff_point, y = diff_point_norm, fill = group_diff_point)) +
  geom_boxplot() +
  labs(title = "Répartition des groupes créés", x = "Groupes", y = "Valeurs de A") +
  theme_minimal()

data_mod1 <- data_mod1 %>% 
  mutate(ecart_classement = case_when(
    group_diff_point == "moins fort" ~ -1,
    group_diff_point == "meme niveau" ~ 0,
    group_diff_point == "plus fort" ~ 1
  ))

data_mod1$ecart_classement <- as.factor(data_mod1$ecart_classement)

data_mod1 %>% 
  mutate(journee_num = as.numeric(str_extract(journee, "[0-9].*")) )%>% 
  ggplot(aes(x = journee_num, y = diff_point_norm, color = group_diff_point)) +
  geom_point(size = 2, alpha = 0.7) +  # Points bien alignés
  labs(title = "Nuage de points des différences de points par journée",
       x = "Journée",
       y = "Différence de points normée") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```


# Analyse descriptive

```{r}
# Graphique pour effet_positif
ggplot(data.frame(Var = names(table(data_mod1$effet_positif)), 
                  Freq = as.numeric(table(data_mod1$effet_positif))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution de effet_positif", x = "Effet positif", y = "Fréquence") +
  scale_fill_manual(values = c("red", "green3")) +
  theme_minimal() +
  theme(legend.position = "none")

# Graphique pour TM_derniere_min
ggplot(data.frame(Var = names(table(data_mod1$TM_derniere_min)), 
                  Freq = as.numeric(table(data_mod1$TM_derniere_min))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution de TM_derniere_min", x = "Temps mort à la dernière minute", y = "Fréquence") +
  scale_fill_manual(values = c("skyblue", "orange")) +
  theme_minimal() +
  theme(legend.position = "none")

# Graphique pour HF
ggplot(data.frame(Var = names(table(data_mod1$HF)), 
                  Freq = as.numeric(table(data_mod1$HF))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution de HF", x = "Hommes ou femmes", y = "Fréquence") +
  scale_fill_manual(values = c("skyblue", "orange")) +
  theme_minimal() +
  theme(legend.position = "none")

# Graphique pour DM_adverse
ggplot(data.frame(Var = names(table(data_mod1$DM_adverse)), 
                  Freq = as.numeric(table(data_mod1$DM_adverse))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution de DM_adverse", x = "2min de l'adversaire", y = "Fréquence") +
  scale_fill_manual(values = c("skyblue", "orange")) +
  theme_minimal() +
  theme(legend.position = "none")

# Graphique statut à 59 minute
ggplot(data.frame(Var = names(table(data_mod1$statut_59_min)), 
                  Freq = as.numeric(table(data_mod1$statut_59_min))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution du statut à 59 min", x = "Statut à 59 min", y = "Fréquence") +
  scale_fill_manual(values = c("brown1", "grey", "lightgreen")) +
  theme_minimal() +
  theme(legend.position = "none")

# Graphique répartition écart de classement

ggplot(data.frame(Var = factor(names(table(data_mod1$group_diff_point)), levels = c("moins fort", "meme niveau", "plus fort")), 
                  Freq = as.numeric(table(data_mod1$group_diff_point))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
  labs(title = "Distribution ecart classement", x = "Ecart classement", y = "Fréquence") +
  scale_fill_manual(values = c("lightgreen", "grey", "brown1")) +
  theme_minimal() +
  theme(legend.position = "none")
```

# Contrastes

```{r}
#contrasts(data_mod1$ecart_classement) <- contr.sum(nlevels(data_mod1$ecart_classement))
data_mod1$ecart_classement <- relevel(data_mod1$ecart_classement, ref = "0")
data_mod1$HF <- factor(data_mod1$HF)
data_mod1$HF <- relevel(data_mod1$HF, ref = 'H')
```

# Modèle global

```{r}
mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + DM_adverse + DM + ecart_classement, 
            data = data_mod1, family = "binomial")
summary(mod1)

mod_final <- stepwise(mod1, direction = "backward/forward", criterion = "AIC")
mod_final

ggcoef_model(mod_final, exponentiate = TRUE)

```


# Modèle égalité

```{r}
data_mod1_egalite <- data_mod1 %>% 
  filter(statut_59_min == 0)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + DM_adverse + DM + ecart_classement + HF, data = data_mod1_egalite, family = "binomial")
summary(mod1)

mod_final_egalite <- stepwise(mod1, direction = "backward/forward", criterion = "AIC")
mod_final_egalite
```

# Modèle perdant

```{r}
data_mod1_perdant <- data_mod1 %>% 
  filter(statut_59_min == -1)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + ecart_classement + HF, data = data_mod1_perdant, family = "binomial")
#summary(mod1)

mod_final_perdant <- stepwise(mod1, direction = "backward/forward", criterion = "AIC")
mod_final_perdant 
```

# Modèle gagnant

```{r}
data_mod1_gagnant <- data_mod1 %>% 
  filter(statut_59_min == 1)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + ecart_classement + HF, data = data_mod1_gagnant, family = "binomial")
#summary(mod1)

mod_final_gagnant <- stepwise(mod1, direction = "backward/forward", criterion = "AIC")
mod_final_gagnant
```


```{r}
ggcoef_compare(list(mod_final_perdant, mod_final_egalite, mod_final_gagnant), exponentiate = TRUE, type = "faceted")

pdf("../Documents/modele_59.pdf", width = 10, height = 7)
ggcoef_compare(
  list(
    "Modèle perdant" = mod_final_perdant,
    "Modèle égalité" = mod_final_egalite,
    "Modèle gagnant" = mod_final_gagnant
  ),
  type = "faceted"
) +
  coord_cartesian(xlim = c(-2,2)) +  # Limiter l'axe des abscisses
  scale_x_continuous(
    breaks = seq(-2,2, by = 1),     # Ajouter des ticks tous les 1
    minor_breaks = seq(-2,2, by = 0.5)  # Sous-ticks tous les 0.5
  )  +
  theme(
    panel.grid.major.x = element_line(color = "grey80", linetype = "longdash"),
    panel.grid.minor.x = element_line(color = "grey90", linetype = "longdash") # Ajouter les lignes pointillées pour les sous-ticks
  )
dev.off()
```


```{r}
# Extraction des coefficients et intervalles de confiance
extract_model_data <- function(model, model_name) {
  coef_data <- broom::tidy(model, conf.int = TRUE)  # Extraire les coefficients et IC
  coef_data$model <- model_name                     # Ajouter le nom du modèle
  #coef_data <- coef_data[coef_data$term != "(Intercept)", ]  # Exclure l'intercept si non pertinent
  coef_data$significant <- coef_data$p.value < 0.05 # Ajouter une colonne pour la significativité
  return(coef_data)
}

# Combiner les données des modèles (sans sous-modèles)
data_plot <- bind_rows(
  extract_model_data(mod_final_perdant, "Modèle P1"),
  extract_model_data(mod_final_egalite, "Modèle P2"),
  extract_model_data(mod_final_gagnant, "Modèle P3")
)

# Créer une colonne de position des variables (termes)
data_plot$term_position <- as.numeric(factor(data_plot$term))

# Créer un tableau de positions horizontales pour chaque terme
term_positions_df <- data.frame(
  term = unique(data_plot$term), 
  term_position = seq_along(unique(data_plot$term))
)

# Joindre ces positions aux données de plot
data_plot <- merge(data_plot, term_positions_df, by = "term", all.x = TRUE)

# Transformer en tibble pour meilleure compatibilité avec ggplot
data_plot <- as_tibble(data_plot)

# Sauvegarde des fichiers CSV
write_csv(data_plot, "data_plot.csv")
write_csv(term_positions_df, "term_positions_df.csv")

# Création du graphique
gg_combined <- data_plot %>%
  filter(term != "(Intercept)") %>% 
  mutate(term = fct_relevel(term, c("HFF", "ecart_classement-1", "ecart_classement1", 
                                    "DM1", "DM_adverse1", "TM_derniere_min_adverse1", "TM_derniere_min1"))) %>%
  ggplot(aes(x = estimate, y = term, xmin = conf.low, xmax = conf.high)) +
  
  # Ligne verticale à x = 0
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", linewidth = 1.5) +
  
  # Barres d'erreur horizontales
  geom_errorbarh(position = position_dodge(width = 0), height = 0.2, color = "darkblue") +
  
  # Points (avec indication de la significativité)
  geom_point(aes(shape = significant), size = 5, color = "darkblue") +  
  
  # Facettes uniquement par modèle
  facet_grid(~model, scales = "free_y", 
             labeller = labeller(model = c("Modèle P1" = "Statut Perdant",
                                           "Modèle P2" = "Statut Egalite",
                                           "Modèle P3" = "Statut Gagnant"))) +
  
  # Personnalisation des formes des points
  scale_shape_manual(
    values = c("TRUE" = 16, "FALSE" = 8),  
    labels = c("Non significatif", "Significatif"),
    name = "Significativité"
  ) +
  
  # Ajustement des axes
  coord_cartesian(xlim = c(-1.5, 1.5)) +
  scale_x_continuous(breaks = seq(-1.5, 1.5, by = 1),
                     minor_breaks = seq(-1.5, 1.5, by = 0.5)) +
  scale_y_discrete(expand = expansion(add = c(0, 0.2))) +
  
  # Thème et mise en forme
  theme_minimal() +
  theme(
    strip.text = element_text(size = 20, face = "bold"),
    axis.text = element_text(size = 22),
    axis.title = element_text(size = 22, face = "bold"),
    panel.spacing = unit(2, "lines"),
    panel.grid.major.y = element_blank(),
    panel.grid.minor.y = element_blank(),
    panel.border = element_rect(color = "black", fill = NA, linewidth = 0.7),
    axis.ticks.y = element_blank(),
    strip.background = element_rect(fill = "lightgray"),
    plot.title = element_text(size = 26, face = "bold"),
    legend.title = element_text(size = 24, face = "bold"),
    legend.text = element_text(size = 22)
  ) +
  
  # Titres et labels
  labs(
    title = "Modèles par statut à la 59ème minute",
    x = "Estimation des coefficients",
    y = "",
    shape = "Significativité"
  ) +

  # Ajouter des lignes horizontales entre les variables explicatives
  geom_hline(data = term_positions_df, aes(yintercept = term_position - 0.5), 
             color = "black", linetype = "solid", linewidth = 0.5)

# Affichage du graphique
gg_combined

ggsave("../Graphiques/modele_59_statut.png", plot = gg_combined, width = 16, height = 11, dpi = 300)
```

```{r}
# Création du graphique
gg_combined_I <- data_plot %>%
  filter(term == "(Intercept)") %>% 
  #mutate(term = fct_relevel(term, c("HFF", "ecart_classement-1", "ecart_classement1", 
  #                                  "DM1", "DM_adverse1", "TM_derniere_min_adverse1", "TM_derniere_min1"))) %>%
  ggplot(aes(x = estimate, y = term, xmin = conf.low, xmax = conf.high)) +
  
  # Ligne verticale à x = 0
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", linewidth = 1.5) +
  
  # Barres d'erreur horizontales
  geom_errorbarh(position = position_dodge(width = 0), height = 0.2, color = "darkblue") +
  
  # Points (avec indication de la significativité)
  geom_point(aes(shape = significant), size = 5, color = "darkblue") +  
  
  # Facettes uniquement par modèle
  facet_grid(~model, scales = "free_y", 
             labeller = labeller(model = c("Modèle P1" = "Statut Perdant",
                                           "Modèle P2" = "Statut Egalite",
                                           "Modèle P3" = "Statut Gagnant"))) +
  
  # Personnalisation des formes des points
  scale_shape_manual(
    values = c("TRUE" = 16, "FALSE" = 8),  
    labels = c("Non significatif", "Significatif"),
    name = "Significativité"
  ) +
  
  # Ajustement des axes
  #coord_cartesian(xlim = c(-1.5, 1.5)) +
  scale_x_continuous(breaks = seq(-3, 3, by = 1),
                     minor_breaks = seq(-3,3, by = 0.5)) +
  scale_y_discrete(expand = expansion(add = c(0, 0.2))) +
  
  # Thème et mise en forme
  theme_minimal() +
  theme(
    strip.text = element_text(size = 18, face = "bold"),
    axis.text = element_text(size = 20),
    axis.title = element_text(size = 20, face = "bold"),
    panel.spacing = unit(2, "lines"),
    panel.grid.major.y = element_blank(),
    panel.grid.minor.y = element_blank(),
    panel.border = element_rect(color = "black", fill = NA, linewidth = 0.7),
    axis.ticks.y = element_blank(),
    strip.background = element_rect(fill = "lightgray"),
    plot.title = element_text(size = 26, face = "bold"),
    # legend.title = element_text(size = 16, face = "bold"),
    # legend.text = element_text(size = 14)
    legend.position = "none"
  ) +
  
  # Titres et labels
  labs(
    #title = "Modèles par statut à la 59ème minute",
    x = "Estimation des coefficients",
    y = "",
    shape = "Significativité"
  ) +

  # Ajouter des lignes horizontales entre les variables explicatives
  geom_hline(data = term_positions_df, aes(yintercept = term_position - 0.5), 
             color = "black", linetype = "solid", linewidth = 0.5)

# Affichage du graphique
gg_combined_I

ggsave("../Graphiques/modele_59_statut_intercept.png", plot = gg_combined_I, width = 16, height = 3, dpi = 300)

```


