...
data_mod1$group_diff_point <- factor(clustering$cluster, levels = c(1,2,3))

table(data_mod1$group_diff_point)

<<<<<<< HEAD
=======
ggplot(data_mod1, aes(x = group_diff_point, y = abs_diff_point_norm, fill = ecart_classement)) +
  geom_boxplot() +
  labs(title = "Répartition des groupes créés", x = "Groupes", y = "Valeurs de A") +
  theme_minimal()
>>>>>>> a96a66c7e23be0fb63b5f9e0696a4e56bed8be29

data_mod1 <- data_mod1 %>% 
  mutate(ecart_classement = case_when(
    group_diff_point == 1 ~ "faible",
    group_diff_point == 2 ~ "moyen",
    group_diff_point == 3 ~ "eleve"
  ))

data_mod1$ecart_classement <- factor(data_mod1$ecart_classement, levels = c("faible","moyen","eleve"))

<<<<<<< HEAD
ggplot(data_mod1, aes(x = ecart_classement, y = abs_diff_point_norm, fill = group_diff_point)) +
  geom_boxplot() +
  labs(title = "Répartition des groupes créés", x = "Groupes", y = "Valeurs de A") +
  theme_minimal()
=======
...
ggplot(data.frame(Var = factor(names(table(data_mod1$ecart_classement)), levels = c("faible", "moyen", "eleve")), 
                  Freq = as.numeric(table(data_mod1$ecart_classement))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
...
```{r}
data_filtre <- data_mod1 %>% filter(HF == "F")
...
Il y a 76% de chances qu'il y ait un effet positif dans la dernière minute du jeu si l'équipe recevant prend un tm, l'équipe adverse n'en prend pas et qu'il y a une différence de point normée de 1 à la 59ème minute.

# Modèle égalité

```{r}
data_mod1_egalite <- data_filtre %>% 
  filter(statut_59_min == 0)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + DM_adverse + DM + ecart_classement, data = data_mod1_egalite, family = "binomial")
#summary(mod1)
...
```{r}
data_mod1_perdant <- data_filtre %>% 
  filter(statut_59_min == -1)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + ecart_classement, data = data_mod1_perdant, family = "binomial")
#summary(mod1)
...
```{r}
data_mod1_gagnant <- data_filtre %>% 
  filter(statut_59_min == 1)

mod1 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + ecart_classement, data = data_mod1_gagnant, family = "binomial")
#summary(mod1)
...
ggcoef_compare(
  ...
  coord_cartesian(xlim = c(-6, 6)) +  # Limiter l'axe des abscisses
  scale_x_continuous(
    breaks = seq(-6, 6, by = 1),      # Ajouter des ticks tous les 1
    minor_breaks = seq(-6, 6, by = 0.5)  # Sous-ticks tous les 0.5
  )
