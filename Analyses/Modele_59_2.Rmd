...
data_mod1$group_diff_point <- factor(clustering$cluster, levels = c(2,3,1))

table(data_mod1$group_diff_point)


ggplot(data_mod1, aes(x = group_diff_point, y = diff_point_norm, fill = ecart_classement)) +
  geom_boxplot() +
...
data_mod1 <- data_mod1 %>% 
  mutate(ecart_classement = case_when(
    group_diff_point == 2 ~ "moins fort",
    group_diff_point == 3 ~ "meme niveau",
    group_diff_point == 1 ~ "plus fort"
  ))
