...
p1 <- data_descr_1 %>%
  ...
  theme(
    ...
    axis.title = element_text(size = 18),                # Titres des axes
    axis.text = element_text(size = 18),                 # Textes des axes
...
ggsave("../Graphiques/graphique_desc_1.png", plot = graphique_1, width = 14, height = 8, dpi = 300)
...
data_summary <- data_mod1 %>%
  ...
  mutate(
    ...
    case_TM = case_when(
      ...
      TM_derniere_min == 0 & TM_derniere_min_adverse == 1 ~ 1,
      TM_derniere_min == 1 & TM_derniere_min_adverse == 0 ~ 2,
    ...
    case_TM_desc = factor(
      ...
      labels = c(
        ...
        "1: TM: Non, TM Adversaire: Oui",
        "2: TM: Oui, TM Adversaire: Non",
...
plot <- ggplot(data_summary, aes(x = as.factor(case_TM), y = pourcentage_positif, fill = case_TM_desc)) +
  ...
  scale_fill_manual(
    values = c("darkorange4", "darkorange", "darkolivegreen", "darkolivegreen1"),  # Couleurs personnalisées
