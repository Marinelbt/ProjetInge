...
```{r}
df <- read.csv("data_final/df_pdfs_final.csv", stringsAsFactors = T, row.names = 1)
...
write_csv(data, "/Users/marine/Desktop/ACO M2 /ProjetInge/Appli/data/data_descr.csv")
...
frequence_ecarts <- data3 %>%
  group_by(difference_points) %>%
  summarise(
    total_instances = n()  
  )

temps_morts_par_ecart_r <- data2 %>%
...
temps_morts_par_ecart_v <- data2 %>%
  filter(action == "Tem" & action_equipe == "v") %>%  
  group_by(difference_points) %>%
  summarise(
    temps_morts = n()  
  )

taux_temps_morts_r <- temps_morts_par_ecart_r %>%
  left_join(frequence_ecarts, by = "difference_points") %>%
  mutate(
    taux_temps_morts = temps_morts / total_instances  
  )

taux_temps_morts_v <- temps_morts_par_ecart_v %>%
  left_join(frequence_ecarts, by = "difference_points") %>%
  mutate(
    taux_temps_morts = temps_morts / total_instances  
  )
...
p_taux_r <- ggplot(taux_temps_morts_r, aes(x = as.factor(difference_points), y = taux_temps_morts)) +
  ...
  labs(
    title = "Taux de prises de temps-morts du club recevant pendant la dernière minute par écart de points",
    x = "Écart de points",
    y = "Taux de Temps-Morts pris à la derière minute"
  ) +
  theme_minimal() +
  theme(
    plot.margin = margin(20, 20, 40, 20),  # Augmenter l'espace en bas (ou autres côtés si nécessaire)
    plot.title = element_text(size = 14)  # Centrer le titre et ajuster la taille
  )

p_taux_r

p_taux_v <- ggplot(taux_temps_morts_v, aes(x = as.factor(difference_points), y = taux_temps_morts)) +
  geom_bar(fill = "skyblue", stat = "identity", show.legend = FALSE) +
  geom_text(
    aes(label = scales::percent(taux_temps_morts, accuracy = 0.1)), 
    vjust = -0.5, 
  ...
  labs(
    title = "Taux de prises de temps-morts pendant la dernière minute par écart de points à la 59ème minute",
    x = "Écart de points",
    y = "Taux de Temps-Morts pris à la derière minute"
  ) +
  theme_minimal()+
...
write_csv(data_summary, "/Users/marine/Desktop/ACO M2 /ProjetInge/Appli/data/data_descr_effetpos.csv")
