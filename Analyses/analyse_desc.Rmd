...
data_pourcentage <- data[data$action == "Tem", ] %>%
  group_by(periode) %>%
...
p2 <- data[data$action == "Tem", ] %>%
  ...
  geom_text(
    stat = "count",
    aes(
      label = scales::percent(..count.. / tapply(..count.., ..x.., sum)[as.character(..x..)], accuracy = 1)
    ),
...
p3 <- ggplot(data = data1[data1$action == "Tem", ], aes(x = as.factor(difference_points), fill = as.factor(difference_points))) +
...
p3bis <- p3 <- ggplot(data = data1[data1$action == "But", ], aes(x = as.factor(difference_points), fill = as.factor(difference_points))) +
  geom_bar(fill = "skyblue") +
  geom_text(
    aes(
      label = scales::percent(..count.. / sum(..count..), accuracy = 1)  
...
distribution_temps_morts <- data1 %>%
  filter(action == "Tem") %>%
  group_by(difference_points) %>%
  summarise(count = n()) %>%
  mutate(
...
combined_data1 <- bind_rows(distribution_ecarts, distribution_temps_morts)

p_combined <- ggplot(combined_data1, aes(x = as.factor(difference_points), y = percentage, fill = type)) +
  ...
  theme(
    ...
    legend.justification = c(1, 1)                       # Alignement en haut à droite
  )
p_combined
...
p4 <- ggplot(data = data1[data1$action == "Tem" & data$Temps_Sec >= 59*60, ], aes(x = as.factor(difference_points))) +
  geom_bar(fill = "skyblue") +
  geom_text(
    aes(
      label = scales::percent(..count.. / sum(..count..), accuracy = 1)  
...
distribution_ecarts <- data2 %>%
  filter(action == "But") %>% 
  group_by(difference_points) %>%
  summarise(count = n()) %>%
  mutate(
    percentage = count / sum(count),
    type = "fréquence des écarts de points"
  )

distribution_temps_morts <- data2 %>%
  filter(action == "Tem") %>%
  group_by(difference_points) %>%
  summarise(count = n()) %>%
  mutate(
...
taux_temps_morts <- temps_morts_par_ecart %>%
  left_join(frequence_ecarts, by = "difference_points") %>%
  mutate(
    taux_temps_morts = temps_morts / total_instances  # Taux de prise de temps-morts
  )

p_taux <- ggplot(taux_temps_morts, aes(x = as.factor(difference_points), y = taux_temps_morts)) +
  ...
  labs(
    title = "Taux de prises de temps-morts par écart de points à la dernière minute du match",
    x = "Écart de points",
...
data_TM <- data_mod1 %>% 
  filter(TM_derniere_min == 1)

data_pasTM <- data_mod1 %>% 
  filter(TM_derniere_min == 0)

data_effect_TM <- data_TM %>%
  group_by(statut_59_min, effet_positif) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(statut_59_min) %>%
  mutate(percentage = count / sum(count) * 100)

data_effect <- data_pasTM %>%
  group_by(statut_59_min, effet_positif) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(statut_59_min) %>%
  mutate(percentage = count / sum(count) * 100)

p6 <- ggplot(data = data_effect_TM, aes(x = as.factor(statut_59_min), y = count, fill = as.factor(effet_positif))) +
  geom_bar(stat = "identity") +
  geom_text(
    aes(
      label = paste0(round(percentage, 1), "%"),
      y = ifelse(effet_positif == 1, count / 2, cumsum(count))  
    ),
    color = "white",
    size = 4,
    position = position_stack(vjust = 0.5)
  ...
  scale_fill_manual(
    values = c("0" = "indianred", "1" = "darkolivegreen3"),
    labels = c("0" = "Non", "1" = "Oui")
  ) +
  labs(
    title = "Effets positifs des temps-morts\n pris à la dernière minute,\nselon le statut à 59 min",
    x = "Statut à 59 min",
    y = "Nombre de Temps-Morts \npris à la dernière minute",
    fill = "Effet positif"
  ) +
...
data_summary <- data_mod1 %>%
  group_by(statut_59_min, TM_derniere_min) %>%
  summarise(
  ...
  mutate(
    ...
    fleche = case_when(
      TM_derniere_min == 1 & statut_59_min == -1 ~ " ↑",  # Flèche vers le haut pour -1
      TM_derniere_min == 1 & statut_59_min %in% c(0, 1) ~ " ↓",  # Flèche vers le bas pour 0 et 1
...
plot <- ggplot(data_summary, aes(x = as.factor(statut_59_min), y = count, fill = effet)) +
  ...
  geom_text(
    data = data_summary %>% filter(effet == "positif"),  # Affiche le texte uniquement pour les barres vertes
    aes(
      label = label_texte,
      y = count / 2  # Positionne le texte au milieu de la barre verte
    ),
    color = "black",
    size = 4,
    fontface = "bold"
  ) +
  facet_wrap(~TM_derniere_min, 
             labeller = labeller(TM_derniere_min = c(`1` = "Avec prise de\nTemps-Mort", `0` = "Sans prise de\nTemps-Mort"))) +
  labs(
  ...
  scale_x_discrete(labels = c("-1" = "Perd", "0" = "Égalité", "1" = "Gagne")) +  # Remplace les labels sur l'axe X
  theme_minimal() +
  theme(
    ...
    strip.text = element_text(size = 12, face = "bold")  # Augmente la taille des titres des facettes
  )
