...
ggplot(data.frame(Var = names(table(data_mod2$statut_29_min)), 
                  Freq = as.numeric(table(data_mod2$statut_29_min))), aes(x = Var, y = Freq, fill = Var)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Freq), vjust = -0.5) +
...
```{r}
mod2 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + diff_point_norm, data = data_mod2, family = "binomial")
#summary(mod2)
...
mod2 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + club_recevant + DM_adverse + DM + diff_point_norm, data = data_mod2_egalite, family = "binomial")
#summary(mod2)
...
mod2 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + HF + DM_adverse + DM + diff_point_norm, data = data_mod2_perdant, family = "binomial")
summary(mod2)
...
mod2 <- glm(effet_positif ~ TM_derniere_min + TM_derniere_min_adverse + HF + DM_adverse + DM + diff_point_norm, data = data_mod2_gagnant, family = "binomial")
summary(mod2)
