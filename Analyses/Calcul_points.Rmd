...
points_par_club <- points_par_club %>% 
  mutate(club = case_when(
    ...
    club == "MOntpellier Handball feminin N" ~ "HB FEMININ MONTPELLIER MEDITERRANEE",
    club == "LES NEPTUNES DE NANTES" ~ "NANTES HANDBALL FEMININ",
    club == "ISSY-PARIS HAND" ~ "PARIS",
    club == "AS LYON CALUIRE" ~ "LYON-CALUIRE HANDBALL",
    club == "JS CHERBOURG MANCHE HB" ~ "CHERBOURG JS MANCHE HB",
    club == "HBC Nantes" ~ "NANTES HBC",
    club == "PAU BILLERE HANDBALL PAU PYRENEES" ~ "PAU BILLERE HANDBALL",
    TRUE ~ club
  )) %>% 
...
```{r}
#write.csv(result, "point_jour_2425.csv")
```
...
```{r}
point_jour <- read.csv("point_jour_2425.csv", row.names = 1, sep=";")
```
...
for (i in 1:nrow(point_jour)){
  ...
  if(is.na(point_jour$J14[i])==T){
    point_jour$J14[i] <- point_jour$J13[i]
  }
...
write.csv(point_jour, "point_jour_final_2425.csv")
```
...
```{r}
df <- read.table("df_pdfs_2425.csv", header = TRUE, sep = ",")
df <- df %>%
  separate(score_final, into = c("score_final_r", "score_final_v"), sep = "-") %>%
  mutate(
    score_final_r = as.integer(score_final_r),
...
point_jour_final <- read.csv("point_jour_final_2425.csv", row.names=1)
...
write.csv(df2, "Data/data_diff_point_norme_2425.csv")
...
```{r}
df2 <- read.csv("Data/data_diff_point_norme_2425.csv", row.names = 1)
...
df_ref_point <- data_mod1 %>% group_by(saison, division, HF, journee) %>% 
