...
# Création d'un index unique pour chaque match
data_mod1 <- data_mod1 %>%
  group_by(code_rencontre) %>%
  mutate(match_index = cur_group_id()) %>%
  ungroup()

# Filtrage pour garder l'équipe R pour les matchs avec un index impair,
# et l'équipe V pour les matchs avec un index pair
data_mod1 <- data_mod1 %>%
  filter(
    (match_index %% 2 == 1 & equipe == "R") |  # Garder l'équipe R pour les index impairs
    (match_index %% 2 == 0 & equipe == "V")    # Garder l'équipe V pour les index pairs
  ) 
