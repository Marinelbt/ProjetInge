...
data <- data %>% 
  mutate(club_recevant = case_when(
    ...
    club_recevant == "ROCHECHOUART-ST-JUNIEN HANDBALL 87" ~ "ROCHECHOUART-ST-JUNIEN HANDBALL",
    club_recevant == "LIMOGES HANDBALL" ~ "LIMOGES HAND",
    TRUE ~ club_recevant
  )) %>% 
  filter(club_recevant != "SAINT-CYR VAR HANDBALL", club_recevant != "ELITE VAL D'OISE")
...
data <- data %>% 
  mutate(club_visiteur = case_when(
    ...
    club_visiteur == "ROCHECHOUART-ST-JUNIEN HANDBALL 87" ~ "ROCHECHOUART-ST-JUNIEN HANDBALL",
    club_visiteur == "LIMOGES HANDBALL" ~ "LIMOGES HAND",
    TRUE ~ club_visiteur
  )) %>% 
  filter(club_visiteur != "SAINT-CYR VAR HANDBALL", club_visiteur != "ELITE VAL D'OISE")
```
...
```{r}
point_jour1 <- read.csv("Data/point_jour.csv", row.names = 1, sep = ";")
point_jour2 <- read.csv("point_jour_N1.csv", row.names = 1, sep = ";")
point_jour3 <- read.csv("point_jour_N1_2.csv", sep = ";")
```
