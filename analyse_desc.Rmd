...
```{r}
df <- read.csv("df_pdfs.csv", stringsAsFactors = T)
...
```{r}
p1 <- ggplot(data = data[data$action == "Tem",], aes(x=as.factor(periode), fill = statut_tm)) +
  geom_bar() +
  labs(
...
p2 <- data %>% filter(Temps_Sec >= 59*60) %>% 
  ggplot(aes(x=as.factor(periode), fill = statut_tm)) +
  geom_bar() +
  labs(
...
dt_5min_av <- data %>%
  ...
  filter(
    Temps_Sec < temps_tem & Temps_Sec >= temps_tem - 5 * 60
  ) %>%
  ungroup()
